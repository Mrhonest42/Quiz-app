...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const page8Disabled = localStorage.getItem('page8Disabled');
    const page8Answer = localStorage.getItem('page8Answer');

    if (storedScore) {
        Score.innerHTML = storedScore;
    }

    if (page8Disabled === 'true') {
        disableAllButtons();
    }

    if (page8Answer) {
        answer.innerHTML = page8Answer;
    }
});
...
buttons.forEach(button => {
    button.addEventListener('click', () => {
        ...
        if (button.textContent === 'Jofra Archer') {
        ...
        localStorage.setItem('page8Answer', answer.innerHTML);

        // Disable buttons
        buttons.forEach(btn => btn.disabled = true);
        localStorage.setItem('page8Disabled', 'true');
    });
});
...
checkAnswer.addEventListener('click', () => {
    ...
    correctAnswer.innerHTML = 'Jofra Archer';
});
