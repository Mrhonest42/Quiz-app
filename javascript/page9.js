...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const page9Disabled = localStorage.getItem('page9Disabled');
    const page9Answer = localStorage.getItem('page9Answer');

    if (storedScore) {
        Score.innerHTML = storedScore;
    }

    if (page9Disabled === 'true') {
        disableAllButtons();
    }

    if (page9Answer) {
        answer.innerHTML = page9Answer;
    }
});
...
buttons.forEach(button => {
    button.addEventListener('click', () => {
        ...
        if (button.textContent === 'Royal Challegers Bangalore') {
        ...
        localStorage.setItem('page9Answer', answer.innerHTML);

        // Disable buttons
        buttons.forEach(btn => btn.disabled = true);
        localStorage.setItem('page9Disabled', 'true');

        // Check Answer button
checkAnswer.addEventListener('click', () => {
    checkAnswer.innerHTML = 'Correct Answer';
    correctAnswer.innerHTML = 'Royal Challegers Bangalore';
});
    });
});
