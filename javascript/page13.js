...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const page13Disabled = localStorage.getItem('page13Disabled');
    const page13Answer = localStorage.getItem('page13Answer');

    if (storedScore) {
        Score.innerHTML = storedScore;
    }

    if (page13Disabled === 'true') {
        disableAllButtons();
    }

    if (page13Answer) {
        answer.innerHTML = page13Answer;
    }
});
...
buttons.forEach(button => {
    button.addEventListener('click', () => {
        ...
        if (button.textContent === 'Kolkalta Knight Riders') {
        ...
        localStorage.setItem('page13Answer', answer.innerHTML);

        // Disable buttons
        buttons.forEach(btn => btn.disabled = true);
        localStorage.setItem('page13Disabled', 'true');

        // Check Answer button
checkAnswer.addEventListener('click', () => {
    checkAnswer.innerHTML = 'Correct Answer';
    correctAnswer.innerHTML = 'Kolkalta Knight Riders';
});
    });
});
