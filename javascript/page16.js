...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const page16Disabled = localStorage.getItem('page16Disabled');
    const page16Answer = localStorage.getItem('page16Answer');

    if (storedScore) {
        Score.innerHTML = storedScore;
    }

    if (page16Disabled === 'true') {
        disableAllButtons();
    }

    if (page16Answer) {
        answer.innerHTML = page16Answer;
    }
});
...
buttons.forEach(button => {
    button.addEventListener('click', () => {
        ...
        if (button.textContent === 'Hardik Pandiya') {
        ...
        localStorage.setItem('page16Answer', answer.innerHTML);
