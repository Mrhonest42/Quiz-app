...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const page12Disabled = localStorage.getItem('page12Disabled');
    const page12Answer = localStorage.getItem('page12Answer');

    if (storedScore) {
        Score.innerHTML = storedScore;
    }

    if (page12Disabled === 'true') {
        disableAllButtons();
    }

    if (page12Answer) {
        answer.innerHTML = page12Answer;
    }
});
...
buttons.forEach(button => {
    button.addEventListener('click', () => {
        ...
        localStorage.setItem('page12Answer', answer.innerHTML);

        // Disable buttons
        buttons.forEach(btn => btn.disabled = true);
        localStorage.setItem('page12Disabled', 'true');

        // Check Answer button
checkAnswer.addEventListener('click', () => {
    checkAnswer.innerHTML = 'Correct Answer';
    correctAnswer.innerHTML = '8';
});
    });
});
