...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const page1Disabled = localStorage.getItem('page1Disabled');
    const page1Answer = localStorage.getItem('page1Answer');

    if (storedScore) {
        Score.innerHTML = storedScore;
    }

    if (page1Disabled === 'true') {
        disableAllButtons();
    }

    if (page1Answer) {
        answer.innerHTML = page1Answer;
    }
});
...
buttons.forEach(button => {
    button.addEventListener('click', () => {
        ...
        if (button.textContent === 'Kane Williamson') {
        ...
        localStorage.setItem('page1Answer', answer.innerHTML);
